...
public class CustomerVehicleDAOimpl implements CustomerVehicleDAO {
    ...
    public boolean update(CustomerVehicle customervehicle) throws SQLException, ClassNotFoundException {
        return SQLUtil.execute("UPDATE Customer_vehicle SET customer_vehicle_id = ?, customer_id = ?,vehicle_number = ? WHERE customer_vehicle_id = ?",customervehicle.getCustomervehicle_id(),customervehicle.getCustomer_id(),customervehicle.getVehicleNumber(),customervehicle.getCustomervehicle_id());
    }
